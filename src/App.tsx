...
export default function App() {
  ...
  // Sync helper to write specific changes directly to the server
  const syncToServer = async (payloadOverrides: any) => {
    // Helper to sync single model changes
    const syncEntity = async (modelName: string, prevList: any[], newList: any[]) => {
      if (!newList) return;
      ...
      for (const item of (prevList || [])) {
        if (!newMap.has(item.id)) {
          console.log(`Direct Save: Deleting ${modelName} entry with id ${item.id}`);
          try {
            const res = await fetch("/api/db/delete", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ model: modelName, id: item.id })
            });
            if (!res.ok) {
              const text = await res.text();
              console.error(`Direct delete of ${modelName} with id ${item.id} failed with status ${res.status}:`, text);
              continue;
            }
            const data = await res.json();
            if (!data.success) {
              console.error(`Direct delete of ${modelName} with id ${item.id} failed:`, data.error);
            }
          } catch (err) {
            console.error(`Network issue deleting ${modelName} with id ${item.id}:`, err);
          }
        }
      }
      ...
      for (const item of (newList || [])) {
        ...
        const hasChanged = !prevItem || JSON.stringify(prevItem) !== JSON.stringify(item);

        if (hasChanged) {
          console.log(`Direct Save: Saving ${modelName} entry with id ${item.id}`);
          try {
            const res = await fetch("/api/save-entry", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ model: modelName, data: item })
            });
            if (!res.ok) {
              const text = await res.text();
              console.error(`Direct save of ${modelName} with id ${item.id} failed with status ${res.status}:`, text);
              continue;
            }
            const data = await res.json();
            if (!data.success) {
              console.error(`Direct save of ${modelName} with id ${item.id} failed:`, data.error);
            }
          } catch (err) {
            console.error(`Network issue saving ${modelName} with id ${item.id}:`, err);
          }
        }
      }
    };

    // Dispatch the correct sync based on keys present in payloadOverrides
    if (payloadOverrides.company_profiles !== undefined) {
      await syncEntity("company_profiles", companyProfiles, payloadOverrides.company_profiles);
    }
    if (payloadOverrides.customers !== undefined) {
      await syncEntity("customers", customers, payloadOverrides.customers);
    }
    if (payloadOverrides.products !== undefined) {
      await syncEntity("products", products, payloadOverrides.products);
    }
    if (payloadOverrides.quotations !== undefined) {
      await syncEntity("quotations", quotations, payloadOverrides.quotations);
    }
    if (payloadOverrides.invoices !== undefined) {
      await syncEntity("invoices", invoices, payloadOverrides.invoices);
    }
    if (payloadOverrides.challans !== undefined) {
      await syncEntity("challans", challans, payloadOverrides.challans);
    }
    if (payloadOverrides.leads !== undefined) {
      await syncEntity("leads", leads, payloadOverrides.leads);
    }
    if (payloadOverrides.subscriptions !== undefined) {
      await syncEntity("subscriptions", subscriptions, payloadOverrides.subscriptions);
    }
    if (payloadOverrides.reminders !== undefined) {
      await syncEntity("reminders", reminders, payloadOverrides.reminders);
    }
    if (payloadOverrides.inventory !== undefined) {
      await syncEntity("inventory", inventoryItems, payloadOverrides.inventory);
    }
  };

  // Initialize and Seed Storage once on mount
  useEffect(() => {
    // 1. Fetch from server-side PostgreSQL database via Prisma
    fetch("/api/db/get")
      .then((res) => res.json())
      .then((resData) => {
  ...
  // Database persistence helper wrappers using direct Prisma sync
