  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      try {
        ...
        // Fallback to static dist directory if dev build exists
    ...
    try {
      // Set up fallback static serving so frontend loads in failsafe mode
      const distPath = path.join(process.cwd(), 'dist');
      app.use(express.static(distPath));
      app.get('*', (req, res) => {
        res.sendFile(path.join(distPath, 'index.html'));
    ...
    // Bind to PORT anyway so the container/process does not crash continuously in dev/deployment
    app.listen(PORT, "0.0.0.0", () => {
